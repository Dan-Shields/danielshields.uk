import lang_colors from './GitHubPinner_lang_colors.json'
...
(function() {
  ...
  // MARK: - Main
  function init() {
    let origin = document.getElementsByClassName("github-pinner")
  ...
  function loadElements(parent, filter = "") {
    ...
    getDataForUrl(values["URL"], values["TYPE"], parent, function(obj, type, element) {
      ...
      if (type == types["PROFILE"]) {
          ...
          if (filter != "") {
            let child = element.querySelector("#gp-container-all-repos")
            let temp = ""
            ...
            temp += "</div>"
            element.innerHTML = temp
            let searchs = document.getElementsByClassName("gp-search")
  ...
  function getDataForUrl(url, type, element, completion) {
    let handler = new APIHandler(url)
  ...
  function parseUrl(url) {
    let profile = /^(http|https):\/\/(www.)?github.com(\/)?\/[A-Za-z\d-]{1,39}(\/)?$/
    let repository = /^(http|https):\/\/(www.)?github.com\/[A-Za-z\d-]{1,39}\/[A-Za-z\d-]{1,100}(\/)?$/
    let repositories = /^(http|https):\/\/(www.)?github.com\/[A-Za-z\d-]{1,39}\?tab=repositories(\/)?$/
    if (profile.test(url)) {
      // profile
      let profileName = url.replace(/^(http|https):\/\/(www.)?github.com(\/)?/g, "").replace(/\/$/, "")
      return {"URL": "https://api.github.com/users/" + profileName, "TYPE" : types["PROFILE"] }
    } else if (repository.test(url)) {
      // repository
      let profileName = url.replace(/^(http|https):\/\/(www.)?github.com(\/)?/g, "").replace(/\/.*(\/)?$/, "")
      let repositoryName = url.replace(/^(http|https):\/\/(www.)?github.com\/[A-Za-z\d-]{1,39}\//g, "").replace(/\/$/, "")
      return {"URL" : "https://api.github.com/repos/" + profileName + "/" + repositoryName, "TYPE": types["REPO"] }
    } else if (repositories.test(url)) {
      // repositories
      let profileName = url.replace(/^(http|https):\/\/(www.)?github.com\//, "").replace(/\?tab=repositories(\/)?$/, "")
