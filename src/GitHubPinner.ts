...
import uomesports_repos from './assets/uomesports_repos.json'
...
function loadElements(parent: HTMLElement, filter = "") {
  ...
  getDataForUrl(values["URL"], values["TYPE"], parent, function(obj: any, type: number, element: HTMLElement) {
    // set up DOM elements
    if (type == types["PROFILE"]) {
        let temp = "<div id=\"gp-container-profile\"><a href=\"" + obj.html_url + "\">"
        ...
        if (filter != "") {
          const child = element.querySelector("#gp-container-all-repos")
          if (!child) return
          let temp = ""
          ...
          let temp = "<input type=\"text\" placeholder=\"Search for project...\" value=\"" + filter + "\" class=\"gp-search\"></input><div id=\"gp-container-all-repos\">"
          for (let j = 0; j < obj.length; j++) {
              temp += getRepo(obj[j])
          }
          temp += "</div>"
...
function getRepo(obj: any) {
  let temp = "<div id=\"gp-container-repo\"><a class=\"gp-title\" href=\"" + obj.html_url + "\">" + obj.name + "</a><p class=\"gp-desc\">" + (obj.description || '') + "</p><div id=\"gp-stats\">"
  if (obj.language != "" && lang_colors[obj.language]) temp += "<p class=\"gp-stat\"><span class=\"gp-lang\" style=\"background-color: " + lang_colors[obj.language]["color"] + ";\"></span>" + obj.language + "</p>"
...
function getDataForUrl(url: string, type: number, element: HTMLElement, completion: (obj: any, type: number, element: HTMLElement) => void) {
    const request = new XMLHttpRequest()
    request.onreadystatechange = function() {
        if (request.readyState == 4 && request.status == 200) {
            completion(JSON.parse(request.responseText), type, element)
        } else if (request.readyState == 4 && url == 'https://api.github.com/users/dan-shields/repos') {
            completion(dan_shields_repos, type, element)
        } else if (request.readyState == 4 && /^(https:\/\/api.github.com\/repos\/dan-shields\/)[A-Za-z\d-]{1,100}(\/)?$/.test(url)) {
            const repoName = url.split('/').slice(-1)[0].toLowerCase()

            const repoData = dan_shields_repos.find(repo => repo.name.toLowerCase() === repoName)
            
            if (repoData) completion(repoData, type, element)
        } else if (request.readyState == 4 && request.status == 403 && /^(https:\/\/api.github.com\/repos\/uomesports\/)[A-Za-z\d-]{1,100}(\/)?$/.test(url)) {
            const repoName = url.split('/').slice(-1)[0].toLowerCase()

            const repoData = uomesports_repos.find(repo => repo.name.toLowerCase() === repoName)
...
function parseUrl(url: string) {
  const profile = /^(http|https):\/\/(www.)?github.com(\/)?\/[A-Za-z\d-]{1,39}(\/)?$/
  const repository = /^(http|https):\/\/(www.)?github.com\/[A-Za-z\d-]{1,39}\/[A-Za-z\d-]{1,100}(\/)?$/
  const repositories = /^(http|https):\/\/(www.)?github.com\/[A-Za-z\d-]{1,39}\?tab=repositories(\/)?$/
  if (profile.test(url)) {
    // profile
    const profileName = url.replace(/^(http|https):\/\/(www.)?github.com(\/)?/g, "").replace(/\/$/, "")
    return {"URL": "https://api.github.com/users/" + profileName, "TYPE" : types["PROFILE"] }
  } else if (repository.test(url)) {
    // repository
    const profileName = url.replace(/^(http|https):\/\/(www.)?github.com(\/)?/g, "").replace(/\/.*(\/)?$/, "")
    const repositoryName = url.replace(/^(http|https):\/\/(www.)?github.com\/[A-Za-z\d-]{1,39}\//g, "").replace(/\/$/, "")
    return {"URL" : "https://api.github.com/repos/" + profileName + "/" + repositoryName, "TYPE": types["REPO"] }
  } else if (repositories.test(url)) {
    // repositories
    const profileName = url.replace(/^(http|https):\/\/(www.)?github.com\//, "").replace(/\?tab=repositories(\/)?$/, "")
